...
void LinearProbingAggregateHashTable<V>::add_batch(int *input_keys, V *input_values, int len)
{
  ...
  for (; i + SIMD_WIDTH <= len;) {
    ...
    int key[SIMD_WIDTH];
    V   value[SIMD_WIDTH];
    selective_load(input_keys, i, key, inv);
    selective_load(input_values, i, value, inv);

    // 2. Update i
    i += __builtin_popcountll(_mm256_movemask_ps(_mm256_castsi256_ps(inv)));

    // 3. Calculate hash
    __m256i hash =
        _mm256_and_si256(_mm256_set1_epi32(capacity_ - 1), _mm256_add_epi32(_mm256_loadu_si256((__m256i *)key), off));
